...
根因背景：gunicorn worker 超时（超时后 SIGKILL）会直接杀掉请求，导致批改
既无结果也无 pending_review 标记（用户看到"没有任何报告"）。模型调用
超时 + JSON 重试叠加后，最坏耗时可能达到 2×模型超时，超过 worker 超时被杀。

本模块提供：
- make_deadline：请求级截止时间（默认 600s，配合 gunicorn --timeout 660）
- expired：是否已到/临近截止
...
# gunicorn --timeout 660；留 60s 给持久化/聚合/响应。
# 用户要求批改最长等待 10 分钟：单模型超时与请求总预算均放开到 600s。
DEFAULT_GRADING_DEADLINE_SECONDS = 600
